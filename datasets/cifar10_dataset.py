...
from .backdoor_dataset import CIFAR10Mem, CIFAR10Pair, BadEncoderTestBackdoor, BadEncoderDataset, ReferenceImg,CIFAR10Pair_bad
import numpy as np
...
def get_pretraining_cifar10(data_dir,ratio):

    train_data = CIFAR10Pair(numpy_file=data_dir + "train.npz", class_type= classes, ratio=ratio,transform=train_transform)
    memory_data = CIFAR10Mem(numpy_file=data_dir + "train.npz", class_type= classes, ratio=ratio,transform=test_transform_cifar10)
    test_data  = CIFAR10Mem(numpy_file=data_dir + "test.npz", class_type= classes,transform=test_transform_cifar10)
    
    return train_data, memory_data, test_data

def get_bad_cifar10(data_dir,ratio,ratio1,trigger_file):
    train_data = CIFAR10Pair_bad(numpy_file=data_dir + "train.npz", class_type= classes, ratio=ratio,ratio1=ratio1,transform=train_transform,trigger_file=trigger_file)
    memory_data = CIFAR10Mem(numpy_file=data_dir + "train.npz", class_type= classes, ratio=ratio,transform=test_transform_cifar10)
    test_data  = CIFAR10Mem(numpy_file=data_dir + "test.npz", class_type= classes,transform=test_transform_cifar10)
