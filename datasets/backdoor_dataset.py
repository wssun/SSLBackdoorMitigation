...
class CIFAR10CUSTOM(Dataset):
    ...
    def __init__(self, numpy_file, class_type,ratio=None,transform=None):
        """
        Args:
            numpy_file (string): Path to the numpy file.
            transform (callable, optional): Optional transform to be applied
        ...
        if ratio == None:
            ...
            end = int(self.input_array['x'].data.shape[0] * ratio)
            self.data = self.input_array['x'][:end]
            self.targets = self.input_array['y'][:,0].tolist()[:end]
        self.classes = class_type
        self.transform = transform
    def __len__(self):
        return self.data.shape[0]

class CIFAR10CUSTOM_bad(Dataset):

    def __init__(self, numpy_file,class_type,ratio=None,trigger_file=None,ratio1=None,transform=None):
        """
        Args:
            numpy_file (string): Path to the numpy file.
            transform (callable, optional): Optional transform to be applied
        ...
        if ratio == None:
            ...
            # end1 = int(self.input_array['x'].data.shape[0] * ratio*(1-ratio1))
            
            self.data = self.input_array['x'][:end]
            # self.data1 = self.input_array['x'][end1:end]
            
            # for img in self.data1:
                
                # img[:] =img * self.trigger_mask_list[0] + self.trigger_patch_list[0][:]
                # self.data = np.append(self.data, img)
            self.targets = self.input_array['y'][:,0].tolist()[:end]
        self.classes = class_type
        self.transform = transform
    def __len__(self):
...
class CIFAR10Pair(CIFAR10CUSTOM):
    ...
    def __getitem__(self, index):
        ...
        return im_1, im_2
class CIFAR10Pair_bad(CIFAR10CUSTOM_bad):
    """CIFAR10 Dataset.
    """
    def __getitem__(self, index):
        img = self.data[index]
        
        if index > self.data.shape[0] * (1-self.ratio1):
